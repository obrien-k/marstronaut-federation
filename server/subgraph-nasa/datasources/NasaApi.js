...
class NasaAPI extends RESTDataSource {
  willSendRequest(request) {
    request.params.set('api_key', process.env.NASA_KEY || 'DEMO_KEY');
  }
  ...
  async getPhotos(rover_name, earth_date) {
    return this.get(
      `mars-photos/api/v1/rovers/${encodeURIComponent(rover_name)}/photos?earth_date=${encodeURIComponent(earth_date)}`
    );
  }
